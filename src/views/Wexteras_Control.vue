<template>
  <div class="container">
    ...
    <div class="sensor-display">
      <p class="overviewTxt">Temperature: <span>{{ currentData?.temperature || 'N/A' }}°C</span></p>
      <p class="overviewTxt">Humidity: <span>{{ currentData?.humidity || 'N/A' }}%</span></p>
    </div>
    ...
    <div class="automation-grid">
      ...
      <div class="automation-card">
        ...
        <input type="number" v-model.number="fanAutomation.tempThreshold" placeholder="Temp Threshold (°C)" @change="updateFanAutomation" />
      </div>
      ...
      <div class="automation-card">
        ...
        <label>
          <input type="checkbox" v-model="hatchAutomation.enabled" @change="updateHatchAutomation" />
          Enabled
        </label>
        <input type="number" v-model.number="hatchAutomation.humThreshold" placeholder="Humidity Threshold (%)" @change="updateHatchAutomation" />
      </div>
      ...
      <div class="automation-card">
        ...
        <input type="number" v-model.number="pumpAutomation.duration" placeholder="Duration (min)" @change="updatePumpAutomation" />
        <input type="number" v-model.number="pumpAutomation.interval" placeholder="Interval (min)" @change="updatePumpAutomation" />
      </div>
    </div>
    ...
    <div class="device-grid">
      ...
      <div v-if="fanData" :style="getColor(fanData.status)" class="device-card">
        ...
        <div v-if="fanData.status">
          ...
          <input type="range" v-model.number="fanData.power" min="0" max="100" @change="updateFanPower" />
        </div>
      </div>
      ...
      <div v-if="pumpData" :style="getColor(pumpData.status)" class="device-card">
        ...
        <div v-if="pumpData.status">
          ...
          <input type="range" v-model.number="pumpData.power" min="0" max="100" @change="updatePumpPower" />
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    setupListeners() {
      onValue(ref(db, '/current'), (snapshot) => {
        this.currentData = snapshot.exists() ? snapshot.val() : { temperature: 'N/A', humidity: 'N/A' }
      })
